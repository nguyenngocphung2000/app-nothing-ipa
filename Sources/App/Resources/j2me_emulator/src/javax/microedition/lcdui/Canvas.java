...
public abstract class Canvas extends Displayable
{
	...
	protected boolean usePaintQueue = false;
	private boolean queuedRepaint = false;
	...
	protected Canvas()
	{
		...
		usePaintQueue = Display.usePaintQueue;
	}
	...
	public void keyPressed(KeyEvent keyEvent) {
		...
		int key = keyEvent.normalizedCode;
		...
		if (listCommands) {
			...
			if (key == Mobile.NOKIA_SOFT1 && commands.size()>0) {
				...
				keyPressed(keyEvent.platformCode);
			}
		}
	}
	...
	public void keyReleased(KeyEvent keyEvent) {
		if (!listCommands) {
			keyReleased(keyEvent.platformCode);
		}
	}

	public void keyRepeated(KeyEvent keyEvent) {
		if (!listCommands) {
			keyRepeated(keyEvent.platformCode);
		}
	}
	...
	public void repaint(int x, int y, int width, int height)
	{
		if (getDisplay().current != this) {
			if (usePaintQueue && queuedRepaint) {
				queuedRepaint = false;
			}
		...
		if (usePaintQueue) {
			if (!queuedRepaint) {
				// slightly not-really-correct, given that the _region_ might not match
				// but in practice it covers the whole screen
				Mobile.getPlatform().requestRepaint(this, x, y, width, height);
				queuedRepaint = true;
			}
		} else {
			// hmm, this is the version that locks user calls...
			...
			try {
				...
				// clip for bar..
				gc.resetWithClip(0, 0, width, fullscreen ? height : height - barHeight);
	...
	public void actuallyRepaint(int x, int y, int width, int height)
	{
		// the version that doesn't lock user calls
		// supposed to be called by the queue
		// we only need to ensure we don't paint over if we're no longer current
		...
		// only queued, so isInsidePaint is impossible
		if (isInsidePaint) {
			// we need this to avoid stackoverflow
			// we shouldn't be here unless paint calls serviceRepaints [wtf???]
			System.out.println("WTF: inside queued paint??");
			Thread.dumpStack();
			Mobile.getDisplay().callSerially(() -> {
				repaint(x, y, width, height);
			});
	...
	private void paintCommandsBar() {
		...
		// restore clip for bar.. video?
		gc.resetWithClip(0, 0, width, fullscreen ? height : height - barHeight);
	}

	// directly called by video
