...
public class ChoiceGroup extends Item implements Choice
{
	...
	protected boolean keyPressed(KeyEvent keyEvent) {
		...
		if (type == Choice.POPUP) {
			if (keyEvent.normalizedCode == Mobile.NOKIA_LEFT && selectedIndex > 0) {
				selectedIndex--;
			} else if (keyEvent.normalizedCode == Mobile.NOKIA_RIGHT && selectedIndex < size()-1) {
				selectedIndex++;
			} else {
				handled = false;
			}
		} else if ((keyEvent.normalizedCode == Mobile.XKEY_SELECT || keyEvent.normalizedCode == Mobile.NOKIA_RIGHT) && hilightedIndex != -1) {
			// space key
	...
	private void _drawTick(CanvasGraphics gc, int index, int height, boolean filled, boolean isCircle) {
